...
export function StatusForm({ open, onOpenChange, status, onSuccess }: StatusFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = status ? `/api/config/statuses/${status.id}` : "/api/config/statuses";
      const method = status ? "PATCH" : "POST";
      ...
      if (!response.ok) {
        ...
        throw new Error(error.error?.message || "Failed to save status");
      }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>Enter status details below.</DialogDescription>
        </DialogHeader>
